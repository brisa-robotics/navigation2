...
public:
    ...
    : BT::ActionNodeBase(service_node_name, conf), service_node_name_(service_node_name)
  {
    ...
    service_client_ = node_->create_client<ServiceT>(service_name_, rmw_qos_profile_services_default, callback_group_);
    ...
    RCLCPP_DEBUG(node_->get_logger(), "\"%s\" BtServiceNode initialized", service_node_name_.c_str());
  }
  ...
  virtual BT::NodeStatus check_future()
  {
    ...
    RCLCPP_WARN(node_->get_logger(), "Node timed out while executing service call to %s.", service_name_.c_str());
    request_sent_ = false;
